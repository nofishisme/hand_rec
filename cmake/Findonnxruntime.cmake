...
``onnxruntime_INCLUDE_DIRS``
 Include directories needed to use onnxruntime.(作为使⽤要求的onnxruntime的头⽂件⽬录)
``onnxruntime_LIBRARIES``
 Libraries needed to link to onnxruntime.(作为使⽤要求的onnxruntime的链接库⽂件路径)
Cache Variables(缓存变量)
^^^^^^^^^^^^^^^^^^^^^^^^^
The following cache variables may also be set(该模块会定义如下缓存变量):
``onnxruntime_INCLUDE_DIR``
 The directory containing ``onnxruntime_c_api.h``.(``onnxruntime_c_api.h``所在⽬录)
``onnxruntime_LIBRARY``
 The path to the onnxruntime library.(onnxruntime库⽂件的路径)
Hints(作为提⽰的查找条件变量)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
``onnxruntime_ROOT`` 
